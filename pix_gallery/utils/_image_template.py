...
class RowStyle(BaseModel):
    font: FreeTypeFont | str | Path | None = "CJGaoDeGuo.otf"
    """字体"""
    font_size: int = 20
    """字体大小"""
...
class ImageTemplate:
    ...
    async def hl_page(
        ...
        返回:
            BuildImage: 图片
        """
        font = BuildImage.load_font("CJGaoDeGuo.otf", 20)
        width, height = BuildImage.get_text_size(head_text, font)
        for title, item in items.items():
            title_width, title_height = cls.__get_text_size(title, font)
            it_width, it_height = cls.__get_text_size(item, font)
        ...
        top_head.line((0, 1, width, 1), "#C2CEFE", 2)
        top_head.text((15, 20), head_text, "#9FA3B2", "center")
        top_head.circle_corner()
        A.paste(top_head, (0, 20), "width")
        ...
        for title, item in items.items():
            ...
            title_background.text((3, 5), title)
            title_background.circle_corner(5)
            _text_width, _text_height = cls.__get_text_size(item, font)
            _width = max([title_background.width, _text_width, _min_width])
            text_image = cls.__build_text_image(
                item, _width, _text_height, font, color="#FDFCFA"
            )
            B = BuildImage(_width + 20, title_height + text_image.height + 40)
            B.paste(title_background, (10, 10))
            B.paste(text_image, (10, 20 + title_background.height))
            B.line((0, 0, 0, B.height), random.choice(cls.color_list))
            A.paste(B, (0, cur_h), "width")
    ...
    def table_page(
        ...
        table_bk.paste(table, center_type="center")
        height = table_bk.height + 170
        background = BuildImage(table_bk.width, height, (255, 255, 255), font_size=50)
        background.paste(table_bk, (0, 170))
        background.text((0, 50), head_text, "#334762", center_type="width")
        if tip_text:
            text_image = BuildImage.build_text_image(tip_text, size=22)
            background.paste(text_image, (0, 110), center_type="width")
        return background
    ...
    def table(
        ...
        for i, data in enumerate(build_data_list):
            column_name_image = BuildImage.build_text_image(
        ...
        for i, data in enumerate(build_data_list):
            ...
            background.paste(column_name_image, (0, 20), center_type="width")
            ...
            for item in data["data"]:
                ...
                if isinstance(item, tuple):
                    ...
                    if isinstance(data, Path):
                        ...
                        image_ = data
                    if image_:
                        background.paste(image_, (padding, cur_h))
                else:
                    background.text(
                        (padding, cur_h),
    ...
    def __build_text_image(
        ...
        cur_h = 0
        for s in text.split("\n"):
            text_image = BuildImage.build_text_image(s, font, font_color=font_color)
            A.paste(text_image, (0, cur_h))
            cur_h += h
        return A
